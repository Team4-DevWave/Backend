...
const handleCastErrorDB = (err) => {
  //transfer weird error into an friendly readable operational error
...
const sendErrorProd = (err, res) => {
  if (err.isOperational) {
    // expected/ trusted error send message to client
    res.status(err.statusCode).json({
      status: err.status,
      message: err.message,
    });
  } else {
    //programming / unknown / 3rd party errors : dont leak error details
    //1)log the error
